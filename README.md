...
```bash
composer install
cp config.example.php config.php   # editar credenciales de tu MySQL local
...
2. **Dominio/subdominio**: configurar `md.pdsx.org` apuntando a una carpeta del hosting (ej. `public_html/md`), y en hPanel fijar el **document root** de ese dominio directamente a la carpeta `public/` del proyecto (no a la raíz) — así `src/`, `vendor/` y `config.php` quedan fuera del árbol servible por el navegador.
3. **Versión de PHP**: en hPanel → PHP Configuration, seleccionar PHP 8.2 o 8.3.
4. **Subir archivos**: vía FTP/File Manager, subir todo el proyecto (incluyendo `vendor/`, ya generado localmente con `composer install`, si el plan no tiene acceso SSH para correr Composer en el servidor).
5. **config.php**: crear en el servidor (fuera de `public/`) copiando `config.example.php` con las credenciales reales de la base de datos de Hostinger. **No subir `config.php` a ningún repositorio público.**
...
- `config.php` — credenciales de conexión (no versionar; usar `config.example.php` como plantilla)
